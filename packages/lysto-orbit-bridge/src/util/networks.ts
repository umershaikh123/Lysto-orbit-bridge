...
export const rpcURLs: { [chainId: number]: string } = {
  ...
  [ChainId.Lysto]: loadEnvironmentVariableWithFallback({
    env: process.env.NEXT_PUBLIC_L3_RPC,
    fallback: `${process.env.NEXT_PUBLIC_L3_RPC}`
  }),
...
export const explorerUrls: { [chainId: number]: string } = {
  ...
  [ChainId.Lysto]: `${process.env.NEXT_PUBLIC_L3_EXPLORER}`,
  // L1
...
export function isNetwork(chainId: ChainId) {
  ...
  const isLysto = chainId === ChainId.Lysto
  ...
  const isEthereumMainnetOrTestnet = isSepolia
  ...
  const isCoreChain = isSepolia
  const isOrbitChain = isLysto

  const isTestnet = isLysto || isSepolia

  const isSupported = isLysto || isSepolia
